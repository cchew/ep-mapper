...
def _lex_au_xml_path() -> Path:
    corpus_dir = Path(os.environ.get("LEX_AU_CORPUS_DIR", "corpus"))
    ...
    if not matches:
        raise FileNotFoundError(
            ...
            "Build it with lex-au:\n"
            "  pip install lex-au\n"
            "  lexau build --type regulation "
            "'Therapeutic Goods (Medical Devices) Regulations 2002'\n"
            "Then set LEX_AU_CORPUS_DIR to the corpus directory."
        )
    if len(matches) > 1:
        print(
            f"WARNING: Multiple XML files match in {corpus_dir}/xml/ — using {matches[0]}",
            file=sys.stderr,
        )
